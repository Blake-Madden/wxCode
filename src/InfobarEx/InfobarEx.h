...
#ifndef _WX_INFOBAR_EX_H_
#define _WX_INFOBAR_EX_H_
...
/**
@brief A rewrite of wxInfoBar with additional features.

Includes the following changes:
- Adds a "don't show this again" checkbox.
- User clicking the Close button now sends the `wxID_CLOSE` event up to the parent so that it can handle it as well.
...
@endcode
*/
class wxInfoBarEx : public wxControl
{
public:
    // The usual ctors and Create() but remember that info bar is created hidden.
    wxInfoBarEx() noexcept : m_icon(nullptr), m_text(nullptr), m_dontShowAgainCheckbox(nullptr), m_includeDontShowAgain(true), m_dontShowAgain(false),
        m_showEffect(wxSHOW_EFFECT_MAX), m_hideEffect(wxSHOW_EFFECT_MAX), m_effectDuration(0)
        {}
    ...
    wxInfoBarEx(wxWindow *parent, wxWindowID winid = wxID_ANY, const bool includeDontShowAgain = true) : m_icon(nullptr), m_text(nullptr), m_dontShowAgainCheckbox(nullptr), m_includeDontShowAgain(includeDontShowAgain), m_dontShowAgain(false),
        m_showEffect(wxSHOW_EFFECT_MAX), m_hideEffect(wxSHOW_EFFECT_MAX), m_effectDuration(0)
        { Create(parent, winid); }
    ...
    /// @returns Whether the "Do not show this again" checkbox was checked at the time of the window being closed.
...
private:
    ...
    wxDECLARE_NO_COPY_CLASS(wxInfoBarEx);
    };

/** @}*/

#endif // _WX_INFOBAR_EX_H_
