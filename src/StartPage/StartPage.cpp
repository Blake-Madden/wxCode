...
wxBEGIN_EVENT_TABLE(wxStartPage, wxWindow)
    EVT_PAINT(wxStartPage::OnPaint)
    EVT_MOTION(wxStartPage::OnMouseChange)
    EVT_LEFT_DOWN(wxStartPage::OnMouseClick)
wxEND_EVENT_TABLE()
...
void wxStartPage::SetMRUList(const wxArrayString& mruFiles)
    {
    m_fileButtons.clear();
    m_fileButtons.resize(mruFiles.GetCount());
    for (size_t i = 0; i < mruFiles.GetCount(); ++i)
        {
...
void wxStartPage::Realise()
    ...
    //calculate how wide the buttons/top label need to be fit their contents
        {
    ...
    // draw the file labels
        ...
        for (size_t i = 0; i < GetMRUFileCount(); ++i)
            ...
            if (m_fileButtons[i].IsOk())
                ...
                wxFileName fn(m_fileButtons[i].m_label);
